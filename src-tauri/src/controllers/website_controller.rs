...
// Add API key validation command
#[tauri::command]
pub async fn test_wpscan_api_key(api_key: String) -> Result<bool, String> {
    if api_key.trim().is_empty() {
        return Err("API key is required".to_string());
    }
    ...
    // Test API key by making a simple request to WPScan API
    let response = client
        .get("https://wpscan.com/api/v3/status")
        .header("Authorization", format!("Token {}", api_key))
        .send()
        .await
        .map_err(|e| format!("Failed to connect to WPScan API: {}", e))?;
...
pub async fn export_website_report(
    ...
    println!("Exporting report for website {} in {} format", website_id, format);
